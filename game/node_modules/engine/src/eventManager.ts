...
class EventManager {
  private event_listeners: object
  private event_queue: object[]
  ...
  dispatch_events(max) {
    ...
    while (
      ...
      const e: any = this.event_queue.shift()
      this.dispatch_event(e.event_type, e.e, e.timestamp)
      ++event_ptr
    }
  }

  push_event(event_type, e, timestamp?: number, immediate?: boolean) {
    const t = timestamp || new Date().getTime()

    if (immediate) {
      return this.dispatch_event(event_type, e, t)
    }

    this.event_queue.push({ event_type, e, t })
  }
