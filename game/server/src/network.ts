...
class Network {
  ...
  private network_event_manager
  private io
  private clients
  private msg_queue
  private msg_bc_queue
  ...
  handle_client_connection(client_socket) {
    ...
    this.event_manager.push_event('net_client_open', { client_id })
    ...
    this.network_event_manager.get_registered_events().forEach((event_type) => {
      client_socket.on(event_type, (e) => {
        ...
        this.network_event_manager.push_event(
          event_type,
          { client_id, e },
          null,
          true
  ...
  handle_client_disconnect(client_id) {
    return () => {
      ...
      this.event_manager.push_event('net_client_close', { client_id })
    }
  }
  ...
  dequeue_all(frame_start_time) {
    this.dequeue_msg(frame_start_time, 0)
    this.dequeue_msg_bc(frame_start_time, 0)
  }
}
